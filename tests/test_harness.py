...
def test_streamlit_dashboard_executes():
    ...
    check("the FYPothesis dashboard has four focused tabs",
          "FYPothesis" in src
    ...
    check("the demo has one plain start action with no replay banner",
          ...
          and "24-second demo" not in src
          and "SIMULATED VIDEO REPLAY" not in src)
    check("the demo remains under 30 seconds",
          "DEMO_DURATION_S = 24" in src)
    check("the demo covers a complete guarded research cycle",
          all(x in src for x in
              ('"BOOT"', '"OBSERVE"', '"QUESTION"', '"PREFLIGHT"',
               '"RECOVER"', '"EVALUATE"', '"CONFIRM"', '"DECIDE"')))
    demo = src.split("def render_demo_run():", 1)[1].split(
        "running = L._agent_running()", 1)[0]
    demo_records = src.split("DEMO_NODE_RECORDS = [", 1)[1].split(
        "def demo_tree_nodes", 1)[0]
    check("the demo uses the exact live-view tree footprint",
          "st.columns([5, 1])" in demo
          and "experiment_tree_chart(nodes)" in demo
          and "height=620" in demo)
    check("the demo journal visibly branches instead of forming one line",
          demo_records.count('"parent": 1') >= 3
          and '"parent": 2' in demo_records
          and '"parent": 5' in demo_records)
    check("the demo mirrors every judge-facing live-run section",
          all(x in demo for x in
              ("Experiment tree", "Experiment brief", "Run summary",
               "Iteration audit trail", "Run reliability and recovery")))
    check("demo tree nodes open their own experiment details",
          "selected_demo_experiment" in demo
          and 'on_select="rerun"' in demo
          and "Configuration and audit evidence" in demo
          and "Executable script sent to the sandbox" in demo)
    check("the Overview has no redundant result-verification button",
          "Verify the submitted result" not in src)
    check("judging evidence is merged into the Overview",
          "Why this is competition-ready" in src
          and "⚖️ Judging criteria" not in src)
    check("the live view renders a clickable journal-backed experiment tree",
          "def experiment_tree_chart" in src and "st.altair_chart" in src
          and 'name="experiment_node"' in src and 'on_select="rerun"' in src)
    check("the tree preserves its compact readable footprint",
          "st.columns([5, 1])" in src and "height=620" in src)
    check("the tree retains journal parent edges and the run root",
          'children = {"start": []}' in src
          and 'else "start"' in src and '"label": "START"' in src)
    check("node selection renders the merged experiment brief and artifacts",
          all(x in src for x in ("selected_experiment_", "Experiment brief",
                                 "Configuration and audit evidence",
                                 "Executable script sent to the sandbox")))
    check("audit, run summary, and robustness are merged into the live view",
          "render_watch_tree()" in src and "render_robustness(" in src
          and "render_iteration_log()" in src and "Run summary" in src
          and "Iteration audit trail" in src)
    combined = src.split("with tabs[2]:", 1)[1].split(
        "# --------------------------------------------------------------------- run ---", 1)[0]
    check("run-specific recovery evidence follows the iteration audit trail",
          combined.index("render_iteration_log()")
          < combined.index("render_robustness(")
          and "Derived only from the selected" in src
          and "Run reliability and recovery" in src
          and "L.run_reliability(journal_path)" in src)

    watch = "@st.fragment(run_every=3)" + src.split(
        "@st.fragment(run_every=3)", 1)[1]
    watch = watch.split("with tabs[2]:", 1)[0]
    check("only the live tree refreshes on a timer",
          "@st.fragment(run_every=3)" in watch and "st.rerun" not in watch)
    ...
    check("...and the app says so explicitly",
          "one-time hidden-test evaluation" in src
          or "hidden-test evaluation runs exactly once" in src)
...
def test_live_view_state():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check("a non-current journal cannot inherit the current run's ledger",
              st["kpi"]["training_runs"] is None
              and st["kpi"]["spend"] is None)
        reliability = L.run_reliability(j)
        check("run reliability groups a failure with its debug descendants",
              reliability["failure_incidents"] == 2
              and reliability["failed_nodes"] == 2
              and reliability["debug_attempts"] == 1)
        check("run reliability separates branch repair from safe continuation",
              reliability["repaired_in_place"] == 1
              and reliability["returned_to_scored_work"] == 1
              and reliability["preflight_rejections"] == 1)
...
def test_results_manifest_is_the_single_source():
    ...
    check("the dashboard reads the manifest instead of hardcoding the score",
          ("from agent import manifest as MF" in app
           or 'read_json(RESULTS / "manifest.json"' in app)
...
def test_training_run_budget():
    ...
    check("the counting rule is stated for the report",
          ...
          and "counts toward the organizer's" in B.COUNTING_NOTE)
...
def test_run_metrics_and_demo():
    ...
    # A preflight rejection uses an organizer iteration but no training run.
    ...
    check("a preflight rejection counts toward the iteration cap",
          m["preflight_rejections"] == 1 and m["iterations_consumed"] == 2
...
def test_budget_accounting():
    """Every node counts as an iteration; compute remains separately counted."""
    print("\n[budget accounting]")
    from agent import budget as B
    ...
    check("a preflight rejection counts toward the organizer iteration cap",
          B.consumes_budget(pre))
    ...
    check("counts capped iterations separately from training outcomes",
          c["iterations_consumed"] == 4 and c["preflight_rejections"] == 2
    ...
    check("preflight retries are tightly capped so one bug cannot use all 50",
