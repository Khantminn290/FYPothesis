...
README, dashboard, results report, and submission documents -- reads from it.

Five distinctions the manifest refuses to blur, because each one has been got
wrong here before:

    validation vs hidden test      the hidden test state and one-shot result
                                   must be reported exactly as recorded
...
def build(journal: str | None = None, run_tests: bool = False) -> dict:
    ...
    return {
        ...
        "convergence": {
            ...
            "caps": conv["caps"],
            # Which checkpoint the OFFICIAL rule scores. The canonical artifact
            # must match this checkpoint before finalization is allowed.
