            ...
            for name in sorted(os.listdir(ab)):
                if name.endswith(".jsonl"):
                    nodes += _read_journal(os.path.join(ab, name),
                                           name.replace(".jsonl", ""))
        # Submission repositories keep the compact autonomous-reproduction
        # journal instead of the historical A/B workspace. It is genuine prior
        # evidence and must remain visible after the development artifacts are
        # removed.
        research = os.path.join(logs, "opus_research")
        if os.path.isdir(research):
            for name in sorted(os.listdir(research)):
                if name.endswith(".jsonl"):
                    nodes += _read_journal(
                        os.path.join(research, name),
                        "research_" + name.replace(".jsonl", ""))
    menu = {}
