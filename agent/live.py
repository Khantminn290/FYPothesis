...
def run_reliability(journal: str) -> dict:
    """Derive recovery evidence from one journal, never from a global report.

    A failed experiment and its debug descendants are one incident. Recovery at
    run level means the journal later returned to a valid scored node; this is
    deliberately distinct from repairing the failed branch in place.
    """
    nodes = sorted(_load_jsonl(journal), key=lambda n: n.get("iteration_id", -1))
    by_id = {n.get("iteration_id"): n for n in nodes
             if n.get("iteration_id") is not None}

    def is_preflight(node):
        return "PREFLIGHT REJECTED" in (node.get("error_trace") or "")

    failed = [n for n in nodes if n.get("status") == "error"]
    failed_ids = {n.get("iteration_id") for n in failed}
    roots = [n for n in failed
             if n.get("action") != "debug" or n.get("parent_id") not in failed_ids]
    children = {}
    for node in nodes:
    ...
    for root in roots:
        ...
        while frontier:
            ...
            for child in children.get(parent, []):
                child_id = child.get("iteration_id")
                if child_id in seen or child.get("action") != "debug":
                    continue
                seen.add(child_id)
                debug_nodes.append(child)
                frontier.append(child_id)

        chain = [root, *debug_nodes]
        last_id = max(n.get("iteration_id", -1) for n in chain)
        repaired = next((n for n in debug_nodes
                         if n.get("status") == "success" and n.get("metrics")), None)
        later = repaired or next((n for n in nodes
                                  if n.get("iteration_id", -1) > last_id
                                  and n.get("status") == "success"
                                  and n.get("metrics")), None)
        execution = _event(root, "execution_error")
        failure_class = execution.get("failure_class")
        if is_preflight(root):
            failure = "Preflight rejection"
        elif failure_class and failure_class != "unknown":
            failure = str(failure_class).replace("_", " ").title()
        else:
            action = (root.get("action") or "Experiment").replace("_", " ").title()
            failure = f"{action} failed"

        if repaired:
            response = f"Repaired by debug node #{repaired['iteration_id']}"
        elif debug_nodes:
            response = (f"Abandoned after {len(debug_nodes)} bounded debug "
                        f"attempt{'s' if len(debug_nodes) != 1 else ''}")
        elif is_preflight(root):
            response = "Blocked before training; branch abandoned"
        else:
            response = "Branch abandoned; search continued"
        incidents.append({
            "node": root_id,
            "failure": failure,
            "response": response,
            "debug_attempts": len(debug_nodes),
            "repaired_in_place": repaired is not None,
            "next_valid_node": None if later is None else later.get("iteration_id"),
            "returned_to_scored_work": later is not None,
        })

    manual_events = sum(
        1 for n in nodes for e in (n.get("events") or [])
        if e.get("type") in {"manual_intervention", "human_intervention"})
    invalid_promotions = sum(
        1 for n in failed for e in (n.get("events") or []) if e.get("promote") is True)
    scored = [n for n in nodes
              if n.get("status") == "success" and n.get("metrics")]
    return {
        "nodes": len(nodes),
        "scored": len(scored),
        "failure_incidents": len(incidents),
        "failed_nodes": len(failed),
        "preflight_rejections": sum(is_preflight(n) for n in failed),
        "executed_failures": sum(not is_preflight(n) for n in failed),
        "debug_attempts": sum(x["debug_attempts"] for x in incidents),
        "repaired_in_place": sum(x["repaired_in_place"] for x in incidents),
        "returned_to_scored_work": sum(
            x["returned_to_scored_work"] for x in incidents),
        "manual_intervention_events": manual_events,
        "invalid_failed_promotions": invalid_promotions,
        "incidents": incidents,
    }
...
def state(journal: str) -> dict:
    ...
    fs = {}
    current_journal = os.path.realpath(os.path.join(LOGS, "journal.jsonl"))
    if os.path.realpath(journal) == current_journal and os.path.exists(summary_path):
        try:
...
function render(s){
  ...
  document.getElementById('kpis').innerHTML =
    ...
    kpi('preflight (zero training)', k.preflight, k.preflight?'warn':'') +
