...
def build(d: dict) -> str:
    ...
    return f"""# FYPothesis — Autonomous ML Research Agent
...
FYPothesis reads its append-only research history, forms a measurable hypothesis,
...
- Streamlit renders the live experiment tree, evidence views, and an explicitly
  armed launcher for new autonomous runs.
...
The one-shot hidden-test gain is real but modest. The submitted model family is
intentionally narrow, Path-B feature discovery did not produce a confirmed Pure
improvement, and the ensemble reduces seed variance rather than adding model
diversity. The bonus KuaiRand-1k and KuaiRand-27k benchmarks were not attempted.
We claim strong capability-transfer autonomy, not independent discovery of an
entirely new learning algorithm.
