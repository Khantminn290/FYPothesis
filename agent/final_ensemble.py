...
from agent import provenance  # noqa: E402
...
def main() -> None:
    ...
    # Reconstruct origin and official eligibility from the canonical journal.
    # A local rebuild reproduces an agent-selected artifact; it must not erase
    # that origin, but it also must not pretend the rebuild itself was an agent
    # decision when the journal does not contain the matching ensemble action.
    origin_agent = False
    try:
        with open(os.path.join(ROOT, "logs", "journal.jsonl")) as fh:
            nodes = [json.loads(line) for line in fh if line.strip()]
        source = next((n for n in nodes
                       if n.get("iteration_id") == r["source_node"]), None)
        origin_agent = bool(
            source and source.get("action") == "ensemble"
            and source.get("status") == "success"
            and round(float((source.get("metrics") or {}).get("primary", 0)), 5)
            == r["primary"])
    ...
    try:
        with open(os.path.join(ROOT, "logs", "final_summary.json")) as fh:
            summary = json.load(fh)
        eligible = ((summary.get("official_convergence") or {})
                    .get("eligible_checkpoint") or {})
        convergence_node = eligible.get("converged_at_node")
        official_eligible = eligible.get("eligible_node") == r["source_node"]
    except (OSError, ValueError):
        pass

    prov = provenance.stamp(
        config=r["config"], seeds=r["seeds_used"],
        code_paths=("agent/final_ensemble.py", "agent/ensemble.py",
                    "agent/executor.py", "runtime/train_lib.py"),
        evaluation="starter-kit evaluate.py on validation only",
        extra={"aggregation": "rank_normalise_then_mean",
               "members_dir": r["members_dir"],
               "source_node": r["source_node"],
               "agent_produced": origin_agent,
               "rebuild_invocation": "python3 -m agent.final_ensemble"})
    r["provenance"] = prov
    r["code_version"] = (prov.get("git") or {}).get("sha") or _git_sha()
    r["timestamp_utc"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
    pdata = prov.get("data") or {}
    r["data_version"] = {
        "fingerprint": pdata.get("sha256"),
        "valid_rows": ((pdata.get("splits") or {}).get("valid") or {}).get("rows")}
    r["evaluator"] = "kuairand-starter-kit/evaluate.py (never modified)"
    # This command creates predictions and evaluates VALIDATION only. A
    # historical one-shot lock may exist, but its presence does not mean this
    # rebuild read hidden labels.
    r["hidden_test_used"] = False
    r["produced_by"] = ("autonomous competition run" if origin_agent else
                        "human-invoked reproducibility rebuild")
    r["official_candidate_node"] = r["source_node"]
    r["official_eligible"] = official_eligible
    r["official_convergence_node"] = convergence_node
