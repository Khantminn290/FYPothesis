...
def official_baselines() -> dict:
    """Read the published comparison rows from the Starter Kit artifact."""
    p = os.path.join(ROOT, "kuairand-starter-kit", "baseline_scores.json")
    doc = _load_json(p) or {}
    return ((doc.get("scores") or {}).get("fm_official") or {})
...
def render(d: dict) -> str:
    ...
    # Deliverable 4 requires the validation-best result and the absolute delta
    # for each benchmark metric, not only the combined primary score.
    valid_base = (d.get("official_baselines") or {}).get("valid") or {}
    if all(isinstance(rep.get(k), (int, float)) and
           isinstance(valid_base.get(k), (int, float))
           for k in ("GAUC", "nDCG@5", "primary")):
        delta = {k: rep[k] - valid_base[k]
                 for k in ("GAUC", "nDCG@5", "primary")}
        L += ["", "### Validation-best result and absolute delta", "",
              "| validation result | GAUC | nDCG@5 | primary |",
              "|---|---:|---:|---:|",
              f"| official baseline | {valid_base['GAUC']:.5f} | "
              f"{valid_base['nDCG@5']:.5f} | {valid_base['primary']:.5f} |",
              f"| **FYPothesis** | **{rep['GAUC']:.5f}** | "
              f"**{rep['nDCG@5']:.5f}** | **{rep['primary']:.5f}** |",
              f"| **absolute delta** | **{delta['GAUC']:+.5f}** | "
              f"**{delta['nDCG@5']:+.5f}** | "
              f"**{delta['primary']:+.5f}** |"]
    ...
    if run.get("tier") == OBSERVED:
        ...
        L += ["| | |", "|---|---|",
              ...
              f"| preflight rejections (zero training) | "
              f"{run.get('preflight_rejections')} |",
