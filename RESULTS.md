...
Generated `2026-08-31T20:43:33Z` by `python3 -m agent.results_report`. Every figure below is read from repository artifacts at generation time; nothing here is retyped from memory.
...
Provenance: commit `656afd3f9777` on `main`, data fingerprint `60fa8bc44d1e3d59`.

### Validation-best result and absolute delta

| validation result | GAUC | nDCG@5 | primary |
|---|---:|---:|---:|
| official baseline | 0.66740 | 0.53570 | 0.60160 |
| **FYPothesis** | **0.67212** | **0.53870** | **0.60541** |
| **absolute delta** | **+0.00472** | **+0.00300** | **+0.00381** |
...
`python3 tests/test_harness.py` → **1127 passed, 0 failed** (22.1s) — **VERIFIED**
...
| preflight rejections (zero training) | 1 |
...
> An outer-loop node is one decision; a training execution is one model actually trained. A paired 3-seed confirmation is 1 node and 6 training executions. A preflight rejection still counts toward the organizer's 50-iteration cap, but uses zero training executions; repeated rejections are additionally capped.
